...
class AdaptiveImageSize
{
     ...
     */
    public function calculate(Imagick $file): void
    {
        ...
        if ($desiredWidth !== "auto" && $desiredHeight === "auto") {
            if ($desiredWidth > $this->srcWidth && !$upScale && !$fit) {
        ...
        elseif ($desiredWidth === "auto" && $desiredHeight !== "auto") {
            if ($desiredHeight > $this->srcHeight && !$upScale && !$fit) {
        ...
        if ($fit) {
            ...
            if (!$upScale && $this->width > $this->srcWidth && $this->height > $this->srcHeight) {
                //debug(1);
                $this->width = $this->srcWidth;
                $this->height = $this->srcHeight;
            }
            elseif (!$upScale && ($this->width > $this->srcWidth || $this->height > $this->srcHeight)) {
                ...
                //debug(['$calc' => $calc]);
                if ($calc->width >= $this->fitWidth && $calc->height >= $this->fitHeight && $fit === "fill") {
                    //debug("1");
                    $this->width = $calc->width;
                    $this->height = $calc->height;
                }
                elseif ($calc->width <= $this->fitWidth && $calc->height <= $this->fitHeight && $fit === "size") {
                    //debug("2");
                    $this->width = $calc->width;
                    $this->height = $calc->height;
                }
    ...
    private function calcDimensions($propFlag, $srcWidth, $srcHeight, $newWidth = "auto", $newHeight = "auto"): stdClass
    {
        $Output = new stdClass();
